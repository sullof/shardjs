...
//		console.log(JSON.stringify(this._keyTypeIndexes));
	...
	timestamp: function (d) {
		// like Unix timestamp
		var ret = d 
			// returns the original time in milliseconds
			? (1000 * this.fromInt62(d)) + this._epoch
			// returns a codified timestamp in int62 format
			: this.toInt62(Math.floor((Date.now() - this._epoch) / 1000));
		return ret;
	},
