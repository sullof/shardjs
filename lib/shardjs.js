...
module.exports = Class.subclass({
	...
	init: function (config) {
		...
		if (config.auth) this._auth = config.auth;
		...
		if (opt.dictionary) 
			for (var j in opt.dictionary) {
				this._dictionary[j] = opt.dictionary[j];
				this._dictionaryInverse[opt.dictionary[j]] = j;
			}
...
//		console.log(JSON.stringify(this._keyTypeIndexes));
	...
	dictionary: function (key) {
		return (this._dictionary[key] || key);
	},
	
	dictionaryInverse: function (key) {
		return (this._dictionaryInverse[key] || key);
	},
	
	minify: function (hash) {
		/*
		 * Calling .translate({name:"John",city:"London"})
		 * return something like
		 * 
		 * {n:"John",c:"London"}
		 * 
		 * using the dictionary set in the config file.
		 * 
		 */
		var ret = {};
		for (var j in hash) 
			ret[this.dictionary[j]] = hash[j];
		return ret;
	},
	
	maxify: function (hash) {
		/*
		 * Does the opposite of .translate
		 * Good to understand what a minified hash means
		 */
		var ret = {};
		for (var j in hash) 
			ret[this.dictionaryInverse[j]] = hash[j];
		return ret;
	}, 
	...
	timestamp: function (d,noInt62) {
		// like Unix timestamp, no milliseconds
		if (d && typeof d != 'number') d = d.getTime();
		var ret = Math.floor(((d ? d : Date.now()) - this._epoch) / 1000);
		return noInt62 ? ret : this.toInt62(ret);
	},

	fromTimestamp: function (d,noInt62) {
		// like Unix timestamp
		return d ? (1000 * (noInt62 ? d : this.fromInt62(d))) + this._epoch : false;
	},

	genKey: function (ktype,quantity,cb) {
		...
		var thiz = this,
			...
			callback = (cb ? cb : quantity) || this._nullfunc;
		
			// The quantity is limited to maximum 62*62 keys
			// If not some of the new keys won't be unique
			quantity = cb ? (quantity < this._variants ? quantity : this._variants) : 1;
			
		this._addVariant(pars,quantity,function (pars) {
			if (quantity == 1) {
				...
				var ret = [],
					m = thiz.toInt62(pars[0].m),
					t = thiz.toInt62(pars[0].t,2) ;
				for (var j=0;j<pars.length;j++) {
					ret[j] = m + thiz.toInt62(pars[j].v,2) + t;
				}
				callback(ret);
			}
		});
	...
	_pseudoVariant: function (pars,quantity,callback) {
		...
		if (quantity == 1) {
			...
			for (var j=0;j<quantity;j++) {
				pv[s]++;
				ret[j] = this._copy(pars);
				ret[j].v = pv[s] % this._variants;
			}
			callback(ret);
		}
	},
	...
	_addVariant: function (pars,quantity,callback) {
		...
		this._pseudoVariant(pars,quantity,callback);
	},
